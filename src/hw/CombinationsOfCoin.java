...
/**
 * 
 * Recursion Combinations Of Coins Given a number of different denominations of
 * coins (e.g., 1 cent, 5 cents, 10 cents, 25 cents), get all the possible ways
...
public class CombinationsOfCoin {
	public List<List<Integer>> combinations(int target, int[] coins) {
		...
		// return res;
		// }
		// don't check target == 0, here, looks like problem requires to return
		// a list of [0, 0, 0, 0...] for target = 0 ?
		ArrayList<Integer> cur = new ArrayList<Integer>();
		helper(target, coins, cur, 0, res);
		return res;
	}

	private void helper(int target, int[] coins, ArrayList<Integer> cur, int idx, ArrayList<List<Integer>> res) {
		if (idx == coins.length - 1) {
			if (target % coins[idx] == 0) {
				cur.add(target / coins[idx]);
				res.add(new ArrayList<Integer>(cur)); // make a copy of cur
				cur.remove(cur.size() - 1); // !!! important, don't forget here
			}
			return;
		}
		int maxCnt = target / coins[idx];
		for (int i = 0; i <= maxCnt; i++) {
			cur.add(i);
			helper(target - i * coins[idx], coins, cur, idx + 1, res);
