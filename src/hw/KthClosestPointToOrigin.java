 ...
 * 
 * Description Given three arrays sorted in ascending order.
 * 
 ...
 * Assumptions
 * 
 * The three given arrays are not null or empty K >= 1 and K <= a.length
 ...
 * 
 * 
 * Medium Breadth First Search Heap
 * 
 * @author
 *
 */
...
public class KthClosestPointToOrigin {
	...
	private long distance(List<Integer> point, int[] a, int[] b, int[] c) {
		long dis = 0L;
		int x = a[point.get(0)], y = b[point.get(1)], z = c[point.get(2)];
		dis += x * x + y * y + z * z;
		return dis;
	}
}
