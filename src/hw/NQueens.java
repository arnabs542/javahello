...
[1, 3, 0, 2] --> 
the Queen on the first row is at y index 1, 
...
[2, 0, 3, 1] --> 
the Queen on the first row is at y index 2, 
...
public class NQueens {
	...
	public List<List<Integer>> nqueensII(int n) {
		...
		// there are 2 * n - 1 diagonals for n * n matrix (each diagonal is x + y = constant in range[0, 2n-2])
		boolean[] usedDiags = new boolean[2 * n - 1];
		// also 2 * n - 1 reversed diagonals, (each diagonal is y - n = constant in range [-n+1, n-1], can shift to +n-1 --> [0, 2n-2])
		boolean[] usedRevDiags = new boolean[2 * n - 1];
		helper(n, 0, cur, usedCols, usedDiags, usedRevDiags, result);
		return result;
	}
	
	private void helper(int n, int idx, int[] cur, boolean[] usedCols, boolean[] usedDiags, boolean[] usedRevDiags, List<List<Integer>> result) {
		if (idx == n) {
	...
	private void mark(int row, int col, int n, boolean[] usedCols, boolean[] usedDiags, boolean[] usedRevDiags, boolean val) {
		usedCols[col] = val;
